...
public class TalkActivity extends Activity implements OnClickListener {
	...
	private static TextView MytextView=null; //Creat a new text view
	...
	private void dis_recive_msg(byte[] tmp_byte) {
		...
		//ChatMsgFmt entity2 = new ChatMsgFmt("Device", tmp, MESSAGE_FROM.OTHERS); //Get the data from BLE4.0
		//chat_list.add(entity2);
		//chat_list_adapter.notifyDataSetChanged();
		MytextView = (TextView)findViewById(R.id.textView1);
		MytextView.setText(tmp);
	}
	...
	//private List<ChatMsgFmt> chat_list = new ArrayList<ChatMsgFmt>(); //List指的是集合.<>是泛型,里面指定了这个集合中存放的是什么数据
	...
	private void initView() {
		...
		// = new ChatAdapater(getApplicationContext());
	...
	private class ChatMsgFmt {
		...
		public ChatMsgFmt(String name, String msg, MESSAGE_FROM from) { //this 对象，它可以在类里面来引用这个类的属性和方法
			this.name = "当前油温";                                           //列表框表头
			this.msg = msg;
			this.from = from;
		}
